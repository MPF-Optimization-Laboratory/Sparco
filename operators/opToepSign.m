...
%   $Id: opToepSign.m 885 2008-04-28 08:30:19Z ewout78 $
...
switch lower(type)
    case 'circular'
       ...
       fun = @(x,mode) opToepGaussCircular_intrnl(df,s,m,n,x,mode);
       op  = classOp(fun);
       
    case 'toeplitz'
       % Generate the entries of the matrix
       k  = max(m,n);
       d  = sign(rand(2*k,1)-0.5) / sqrt(m); d(m+1:end-n+1) = 0;
       df = fft(d);
       ...
       fun = @(x,mode) opToepGauss_intrnl(df,s,m,n,x,mode);
       op  = classOp(fun);
...
if mode == 0
    y = {m,n,[0,1,0,1],{'opToepGauss','toeplitz'}};
elseif mode == 1
    k = max(m,n);
...
if mode == 0
    y = {m,n,[0,1,0,1],{'opToepGauss','circular'}};
elseif mode == 1
