...
% 28 Jul 09: Updated to use Spot operators.
%
%   Copyright 2008, Rayan Saab, Ewout van den Berg and Michael P. Friedlander
%   http://www.cs.ubc.ca/labs/scl/sparco
%   $Id: prob401.m 1679 2010-04-29 23:26:14Z mpf $
...
mixing      = getOption(parm, 'mixing', [0.1375, 0.7250; 0.9905, 0.6888]);
info.name   = 'srcsep1';
...
% Determine number of blocks and adjust signal length
nBlocks     = floor(length(s1) / 256) - 1;
s1          = s1(1:(nBlocks+1)*256);
...
% Set up the problem
data.signal = [s1, s2];
data.B      = opBlockDiag(2, opBlockDiag(nBlocks, opDCT(512), 256));
data.M      = opMatrix(kron(mixing,speye(m,m)), ...
