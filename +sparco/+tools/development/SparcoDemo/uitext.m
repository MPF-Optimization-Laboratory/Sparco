...
data.SetVisibility = @OnSetVisibility;
...
% ---------------------------------------------------------------------------
function OnSetVisibility(hObject,state)
% ---------------------------------------------------------------------------
data  = get(hObject, 'UserData');

% Set visibility of text lines
for i=1:length(data.hText)
   set(data.hText(i),'visible',state);
end

% Set visibility of scrollbar
if strcmp(state,'off')
   set(hObject,'visible',state);
else
   OnUpdateSlider(data.hScroll)
end
